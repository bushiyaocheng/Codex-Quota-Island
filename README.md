...
把 Finder、桌面、截图工具或其他应用中的图片和文件拖到灵动岛，岛体会切换为只显示文件的临时托盘并保持展开。可以逐个拖出，也可以拖动顶部的“全部”，在一次原生拖拽中把所有暂存文件送到 Codex；成功后对应暂存项会自动移出，最后一个文件移出后灵动岛恢复原来的额度状态。

- 支持一次拖入多个文件
- 多文件时支持拖动“全部”一次拖出整批文件
- 图片显示缩略图，其他文件显示系统图标
- 支持通过 `+` 或右键菜单选择文件
- 支持单项移除和一键清空
- 仅在当前运行会话中暂存，退出应用后自动清空
- 从不复制或删除原始文件；只有其他应用提供的临时图片表示会复制到系统临时目录，拖出后短暂保留供接收方读取，并在 5 分钟内、清空或退出时删除
...
- 拖入文件后在紧凑态显示暂存数量
- 从暂存托盘逐个或整批把原生文件拖到 Codex 输入框，成功后自动移出
...
- 移除原文件的暂存记录不会删除原文件
- 文件清单不做持久化；临时图片副本拖出后最多保留 5 分钟，并会在退出时清理
- 不上传认证信息或额度数据
- 不执行额度重置；可用重置次数仅供展示
- 不包含第三方分析 SDK
