...
把 Finder、桌面、截图工具或其他应用中的图片和文件拖到灵动岛，岛体会自动展开并把内容加入本地暂存托盘。需要发送时，把托盘里的缩略图直接拖到 Codex 输入框即可。

- 支持一次拖入多个文件
- 图片显示缩略图，其他文件显示系统图标
- 支持通过 `+` 或右键菜单选择文件
- 支持单项移除和一键清空
- 暂存路径会在应用重启后恢复；原文件失效时自动移除
- 从不复制或删除原始文件；只有其他应用提供的临时图片表示会复制到 Codex Island 的本地暂存目录
